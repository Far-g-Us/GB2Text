...
def detect_multiple_languages(rom_data: bytes, start: int = 0, length: int = 2000,
    ...
    CYRILLIC_UPPER (0x80-0xAF), а HIRAGANA_RANGE (0x80-0x9F) — с CP866
    прописными буквами (0x80-0x9F). В 8-битных таблицах один и тот же байт
    может быть катаканой или кириллицей. Когда плотности японского
    и русского высоки одновременно, приоритет отдаётся русскому при наличии
    эксклюзивных CP866 байт (0xE0-0xFD), иначе японскому.
...
def _setup_russian_charmap(charmap: dict):
    """Настоящая таблица CP866 (DOS Russian), сверена с кодеком побайтово.

    Контракт: значения букв/символов уникальны (один символ — один байт,
    first-wins для reverse-map при encode); ASCII 0x20-0x7E совпадает
    с CP866; 0xFF ЯВНО '\n' (игровой терминатор, как в en/ja-таблицах).
    BREAKING 1.4: старая game-specific пример-таблица заменена стандартом.
    """

    # CP866: 0x80-0x9F А-Я, 0xA0-0xAF а-п, 0xE0-0xEF р-я,
    # 0xF0/0xF1 Ё/ё, 0xF2-0xF7 ЄєЇїЎў, 0xB0-0xDF псевдографика,
    # 0xF8-0xFE символы °∙·√№¤■.
    cyrillic_map = {
    ...
    # ASCII printable совпадает с CP866 (как в _setup_english_charmap):
    # смешанный RU+EN текст иначе дропается как unknown.
    for byte in range(0x20, 0x7F):
        charmap[byte] = chr(byte)

    # 0xFF мапим ЯВНО на '\n' (а не эвристикой _setup_common_symbols):
    # это стандартный терминатор сообщений в GB-играх (как в en/ja-таблицах),
    # иначе сообщения склеиваются; encode('\n') детерминирован.
    charmap[0xFF] = '\n'
...
def _setup_japanese_charmap(charmap: dict, freq: Counter):
    ...
    if hiragana_used and not katakana_used:
        ...
        hiragana_map = {
            ...
            0xB6: 'ゕ', 0xB7: 'ゖ',
            # 0xB8/0xB9/0xC7 УДАЛЕНЫ (дубли 0x84 '゛', 0x85 '゜', 0x86 'ー'):
            # дубли значений ломают детерминизм reverse-map при encode
            # (см. F2/CP866); декод этих байтов теперь unknown.
            0xBA: 'ゝ', 0xBB: 'ゞ',
            0xBC: 'ゟ', 0xBD: '゠', 0xBE: 'ァ', 0xBF: 'ィ', 0xC0: 'ゥ', 0xC1: 'ェ',
            0xC2: 'ォ', 0xC3: 'ャ', 0xC4: 'ュ', 0xC5: 'ョ', 0xC6: 'ッ'
        }
