...
### Качество
- [x] Полное покрытие тестами — **100% statements+branches** (core, plugins, api — 87 файлов, 2351 тест, `branch=True`; `gui/*` и `plugins/gba_golden_sun` в `omit` как GUI/ROM-специфика, `pragma: no cover/branch` только на доказуемо мёртвых ветках)
...
- [x] DTE-словарь compression helper (ЭКСПЕРИМЕНТАЛЬНО — тестовый, синтетика + in-memory пилот): построитель байтовых DTE-таблиц под конкретный перевод с атомарным coder-контрактом, гейтами disjoint/compressed-отказов и оценкой экономии; запись в injector — follow-up

### Шрифты (ЭКСПЕРИМЕНТАЛЬНО — всё требует тестов на реальных ROM)
- [x] Фаза 0/1: исследование форматов шрифта и базовые операции с тайлами (декодирование, превью)
- [x] F1-ядро: глубины 1/2/4 бит на пиксель, импорт/экспорт через картинки, строгая проверка метаданных, гейты безопасной записи
- [x] Фаза 2: механизм встраивания глифов с подтверждениями и отказами (сжатые данные, выход за границы, кандидаты)
- [ ] Пилот: прогон всего пайплайна на реальных ROM с проверкой в эмуляторе
...
### Шрифты: тестирование F1 + GUI-редактор
- [ ] Пилот F1 на реальных ROM (legally-owned): candidate-гейт + inject + проверка в эмуляторе, снятие статуса экспериментального
- [x] F2 GUI-редактор, MR1: вкладка «Шрифт» — превью сетки глифов, импорт PNG, запись через TextInjector (read-only без ROM, undo до save, тёмная тема) — код готов, нужен пилот
- [x] F2 GUI-редактор, MR2: попиксельное редактирование глифов + undo/redo стеки — код готов, нужен пилот

### Playtest-ассистент (после F2, по очереди)
- [x] `core/playtest.py`: PlaytestRunner-протокол + PyBoyBackend (GB/GBC) — input-скрипты со строгой валидацией, покадровые хеши, stuck/dark-маркеры, скриншоты на метках, структурный diff (только roundtrip), 100% покрытие на fake-ядре — код готов, нужен пилот на реальном ROM
- [ ] MGBABackend (GBA): Lua-драйвер + сборщик скриншотов готовы (API пин 0.10.x, frame-контракт как PyBoy, пилот-гейт); автопрогон — вторым инкрементом после пилота
...
- Запечённый в графику текст (титульные логотипы как тайлы — отдельный пайплайн от шрифта, исследование)
...
- [x] Дифф-эндпоинт `/diff` — сравнение текста между версиями ROM (реализован в 1.4: `core/diff_text.py` + `POST /diff`)
